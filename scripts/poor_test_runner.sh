...
run_tests() {
    if [[ $TRAVIS_JOB_NAME = "Cryptol Docker on Linux Host - Test Snippet Extraction" ]]; then
        CRYPTOL_ALIAS="docker run -v $(pwd):/mnt/cryptol-course --env CRYPTOLPATH=/mnt/cryptol-course galoisinc/cryptol:2.10.0" \
    ...
    ls ${icry_list[@]} | while read -r ICRY ; do
        ...
        $CRYPTOL_ALIAS -b $CRYPTOL_MNT$ICRY | tee $ACTUAL
